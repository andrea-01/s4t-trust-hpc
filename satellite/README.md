...
- **Supporto Trasparente a Nuovi Dispositivi per il Leasing**: Qualunque board registrata su IoTronic e marcata `online`, una volta approvata on-chain (es. tramite allowlist o approvazione dinamica), diventa immediatamente leasable dal satellite senza alcun riavvio o riconfigurazione.
- **Vincolo Esplicito sul Backend di Calcolo (Limite Noto)**: Il satellite richiede che ciascun nodo assegnato a una pipeline abbia un backend di calcolo dedicato esplicitamente risolvibile (`{device_id}:50051`, oggi configurato per i device `worker-*`). Se una board priva di un proprio container worker dedicato (es. `test_board`) viene inclusa nell'esecuzione di un task, il satellite rifiuta esplicitamente l'esecuzione sollevando un'eccezione (`Nessun backend di calcolo configurato per il device...`), evitando collisioni silenziose o alias su container condivisi. L'esecuzione indipendente su board dinamiche arbitrarie con motore C++ dedicato resta demandata a future estensioni infrastrutturali.
