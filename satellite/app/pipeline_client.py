...
async def run_pipeline_task(pipeline_id: str, nodes: List[str], initial_value: int) -> Dict[str, Any]:
    ...
    for device_id in nodes:
        if not device_id.startswith("worker-"):
            raise Exception(f"Nessun backend di calcolo configurato per il device {device_id}: solo i device worker-* hanno un worker_addr assegnato")
